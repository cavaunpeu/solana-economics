...
unstaked_policy = st.sidebar.selectbox("Unstaked Policy", ("Constant", "Proactive"))

staked_policy = st.sidebar.selectbox("Staked Policy", ("Constant", "Proactive"))

st.sidebar.markdown("## Proactive Policy Parameters")

yield_location = st.sidebar.slider(
    "Yield Location", 0.0, 0.1, C["yield_location"], 0.01
)
yield_scale = st.sidebar.slider("Yield Scale", 10.0, 50.0, C["yield_scale"], 10.0)
...
init_supply = (
    st.sidebar.slider(
        "Initial SOL supply (100M)", 1.0, 10.0, C["initial_supply"] / 1e8, 1.0
    )
    * 1e8
)
...
init_perc_staked = (
    st.sidebar.slider(
        "Initial percentage staked",
        0.0,
        100.0,
        C["initial_fraction_staked"] * 100.0,
        0.5,
    )
    / 100
)
...
base_infl_rate = (
    st.sidebar.slider(
        "Base inflation rate", 0.0, 20.0, C["base_inflation_rate"] * 100.0, 0.5
    )
    / 100
)
dis_infl_rate = (
    st.sidebar.slider(
        "Disinflation rate", -20.0, 0.0, C["disinflation_rate"] * 100.0, 0.5
    )
    / 100
)
long_term_infl_rate = (
    st.sidebar.slider(
        ...
        0.0,
        20.0,
        C["long_term_inflation_rate"] * 100.0,
        0.5,
    )
    / 100
)
...
vdtr_comm_perc = (
    st.sidebar.slider(
        ...
        0.0,
        100.0,
        C["validator_commission_fraction"] * 100.0,
        0.5,
    )
    / 100
)
vdtr_uptime_freq = (
    st.sidebar.slider(
        "Validator uptime frequency",
        0.0,
        100.0,
        C["validator_uptime_frequency"] * 100.0,
        0.5,
    )
    / 100
)
...
simulation = CadCadSimulationBuilder.build(
    ...
    partial_state_update_blocks=[
        {
            "policies": {"staker_behavior": p_staker_behavior},
            "variables": {
...
with st.expander("See description"):
    ...
    description(yield_location, yield_scale)
...
for i in range(num_steps if run_simulation else 1):
    ...
    for ((stat, meta), col) in zip(stat2meta.items(), cols):
        ...
        with col:
            st.metric(
                label=meta["label"], value=meta["format_func"](row[stat]), delta=delta
            )
    # Update plots
    if i == 0:
        ...
        with col2:
            valuation_chart = ValuationAltairChart.build(
                row, num_steps, INITIAL_VALUATION
            )
    else:
